...
\par
I assume that there is some $k\in\N$ such that we can project our hurricanes into an $\R^{k}$ vector space, where the Euclidean distance between two hurricanes in $\R^{k}$ is approximately equal to their dissimilarity.
To compute the projection into the $\R^{k}$ space, I use kernel principal component analysis (KPCA)~\cite{scholkopf1997kernel}, using the $\mathcal{S}$ as a precomputed kernel.
